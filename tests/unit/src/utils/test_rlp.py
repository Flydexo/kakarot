    ...
    return list(
        encode(
            [
                ...
                "0x",
                "0x1b",
                "0x110d8fee1de53df0870e6eb599ed3bf68fb3f1e62c82dfe5976c467c97253b15",
                "0x3450b73d2aef2009f026bcbf097a257ae7a37eb5d3b73dc0760aefad2b98e327",
            ]
        )
    )
...
@pytest.fixture
def short_list():
    # rlp lib uses always long lists
    # used https://toolkit.abdk.consulting/ethereum#rlp
    # to encode: ["0x","0x0430e23400","0x5208","0x01","0x","0x1b","0x110d8fee1de53df0870e6eb599ed3bf68fb3f1e62c82dfe5976c467c97253b15"]
    return list(
        bytes.fromhex(
            "ee80850430e2340082520801801ba0110d8fee1de53df0870e6eb599ed3bf68fb3f1e62c82dfe5976c467c97253b15"
        )
    )
    ...
    class TestRLPListEncode:
        async def test_should_encode_list_longer_55_bytes(self, rlp, long_list):
            rlp_list = await rlp.test__encode_list(
                long_list[1 + long_list[0] - 0xF7 :]
            ).call()
            assert long_list == rlp_list.result.data

        async def test_should_encode_list_smaller_55_bytes(self, rlp, short_list):
            rlp_list = await rlp.test__encode_list(short_list[1:]).call()
            assert short_list == rlp_list.result.data
    ...
    class TestRLPDecode:
        @pytest.mark.parametrize(
            ...
            [
                ...
                random.randbytes(random.randint(2, 54) * 8),
                random.randbytes(random.randint(56, 10000) * 8),
